...
export class DossierRmqDialogComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.dossierRmq.id === undefined) {
            this.dossierRmq.assuree=new Assuree(null,{},null);
    ...
    savePersonne() {
        if (this.dossierRmq.assuree.personne.id !== undefined) {
            this.subscribeToSaveResponsePersonne(
                this.personneService.update(this.dossierRmq.assuree.personne));
        } else {
            this.subscribeToSaveResponsePersonne(
                this.personneService.create(this.dossierRmq.assuree.personne));
        }
    }
    ...
    saveTypeService() {
        ...
        if (this.dossierRmq.typeService.id !== undefined) {
            this.subscribeToSaveResponseTypeService(
                this.refTypeServiceService.update(this.dossierRmq.typeService));
        } else {
            this.subscribeToSaveResponseTypeService(
                this.refTypeServiceService.create(this.dossierRmq.typeService));
        }
    }

    private subscribeToSaveResponseTypeService(result: Observable<RefTypeService>) {
        result.subscribe((res: RefTypeService) =>
            this.onSaveSuccessTypeService(res), (res: Response) => this.onSaveError(res));
    }

    private onSaveSuccessTypeService(result: RefTypeService) {
